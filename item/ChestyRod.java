...
public class ChestyRod extends Item {

	public ChestyRod(int par1) {
		super(par1);
		maxStackSize = 1;
		setFull3D();
		setItemName("chestyRod");
	...
	public boolean onItemUse(ItemStack itemStack, EntityPlayer player, World world, int par4, int par5, int par6, int par7, float par8, float par9, float par10) {
		if(!world.isRemote) {	
			...
			if(!player.isSneaking() && !itemStack.getTagCompound().hasKey("ChestyEntity")) {
				...
				entityChesty.chestyRod = itemStack;
				return true;
			}
		}
        return false;
	...
	public boolean onDroppedByPlayer(ItemStack item, EntityPlayer player) {
		if(!player.worldObj.isRemote && item.hasTagCompound()) {
			EntityChesty chesty;
			if((chesty = (EntityChesty) player.worldObj.getEntityByID(item.getTagCompound().getInteger("ChestyEntity"))) != null) {
				chesty.setDead();
			}
		}
		return true;
