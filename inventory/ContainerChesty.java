...
public class ContainerChesty extends Container {
	...
	public ContainerChesty(InventoryPlayer inventoryPlayer, EntityChesty chesty) {
		this.chesty = chesty;
		chesty.openChest();
		this.addSlotToContainer(new Slot(chesty, 0, 8, 18));
		this.addSlotToContainer(new Slot(chesty, 1, 26, 18));
		for(int i = 0; i < 4; i++) {
		...
		for(int var3 = 0; var3 < chesty.getSizeInventory()/9; ++var3) {
			for(int var4 = 0; var4 < 9; ++var4) {
				this.addSlotToContainer(new Slot(chesty, var4 + var3 * 9 + EntityChesty.SPECIAL_SLOTS_SIZE, 8 + var4 * 18, 40 + var3 * 18));
			}
		}
	...
	public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int slot) {
		...
		if(var4 != null && var4.getHasStack()) {
			...
			if(slot > EntityChesty.SPECIAL_SLOTS_SIZE-1) {
				...
				if(var5.getItem().isItemTool(var5)) {
					canSortInventory = !this.mergeItemStack(var5, 0, 1, false);
					if(!canSortInventory)
						var4.putStack((ItemStack) null);
				}
			}
